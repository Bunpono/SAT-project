...
export default function AdminDashboard() {
  ...
  if (status === "loading") {
    return <section className="rounded-2xl border border-[#E5E7EB] bg-white p-8 text-[#374151] shadow-[0_18px_50px_rgba(17,24,39,0.08)] transition-all duration-300 dark:border-[#263042] dark:bg-[#111827] dark:text-[#D1D5DB] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]">Loading admin data...</section>
  }

  if (status === "error") {
    return (
      <section className="rounded-2xl border border-red-200 bg-white p-8 text-red-700 shadow-[0_18px_50px_rgba(17,24,39,0.08)] transition-all duration-300 dark:border-red-900 dark:bg-[#111827] dark:text-red-300 dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]">
  ...
  return (
    <section className="space-y-6">
      <div className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-[0_18px_50px_rgba(17,24,39,0.08)] transition-all duration-300 sm:p-8 dark:border-[#263042] dark:bg-[#111827] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]">
            ...
            <tbody>
              {history.map((item) => (
                <tr key={item.id} className="border-b border-[#E5E7EB] text-[#374151] transition-colors duration-300 dark:border-[#263042] dark:text-[#D1D5DB]">
                  <td className="px-3 py-3"><span className="font-semibold text-[#111827] dark:text-white">{item.user.name}</span><br /><span className="text-xs text-[#6B7280] dark:text-[#9CA3AF]">{item.user.email}</span></td>
          ...
          {reports.map((report) => (
            <article key={report.id} className="rounded-2xl border border-[#E5E7EB] bg-[#F7F8FC] p-4 text-[#374151] transition-all duration-300 dark:border-[#263042] dark:bg-[#151B2D] dark:text-[#D1D5DB]">
              <div className="flex flex-col gap-1 sm:flex-row sm:justify-between">
                <p className="font-semibold text-[#111827] dark:text-white">{report.user.name} <span className="font-normal text-[#6B7280] dark:text-[#9CA3AF]">({report.user.email})</span></p>
